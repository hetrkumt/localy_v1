...
public class ReviewService {
    ...
    public Mono<Review> findReviewById(Long id) {
        ...
        return reviewRepository.findById(id) // Mono<Review> 반환
                ...
                .doOnSuccess(review -> System.out.println("--- ReviewService: 리뷰 찾음 (ID: " + id + ") ---"))
                .doOnError(e -> System.err.println("--- ReviewService: findReviewById 오류 - " + e.getMessage() + " ---"));
    }
    ...
    public Flux<Review> findReviewsByStoreId(Long storeId) {
        ...
        return reviewRepository.findByStoreId(storeId) // Flux<Review> 반환
                .doOnComplete(() -> System.out.println("--- ReviewService: findReviewsByStoreId 완료 ---"))
                .doOnError(e -> System.err.println("--- ReviewService: findReviewsByStoreId 오류 - " + e.getMessage() + " ---"));
    }
    ...
    public Flux<Review> findReviewsByUserId(String userId) {
        ...
        return reviewRepository.findByUserId(userId) // Flux<Review> 반환
                .doOnComplete(() -> System.out.println("--- ReviewService: findReviewsByUserId 완료 ---"))
                .doOnError(e -> System.err.println("--- ReviewService: findReviewsByUserId 오류 - " + e.getMessage() + " ---"));
    }


    // 새로운 리뷰 제출 (생성)
    // Review 객체에는 storeId, userId, rating, comment가 채워져 있다고 가정합니다.
    // createdAt/updatedAt은 Auditing 설정에 의해 자동 처리되는 것이 이상적입니다.
    ...
    public Mono<Review> updateReview(Long id, Review updatedReview, String currentUserId) {
        ...
        return reviewRepository.findById(id) // Mono<Review> 반환
                .switchIfEmpty(Mono.error(new NoSuchElementException("Review not found with ID: " + id))) // 없으면 예외 발행
                .flatMap(existingReview -> { // 리뷰를 찾으면 다음 단계 실행
                    ...
                    if (!existingReview.getUserId().equals(currentUserId)) {
                        ...
                        // 권한이 없으면 SecurityException과 같은 적절한 예외 발행
                    ...
                    // storeId, userId는 수정 불가 필드
                    // updatedAt은 Auditing 설정 시 자동 업데이트

                    // 4. 업데이트된 엔티티를 저장합니다.
                    return reviewRepository.save(existingReview); // save는 저장 후 업데이트된 Mono<Review> 반환
                })
        ...
        return reviewRepository.findById(id) // Mono<Review> 반환
                ...
                .flatMap(existingReview -> { // 리뷰를 찾으면 다음 단계 실행
                    ...
                    return reviewRepository.deleteById(id)
                            ...
                            .doOnError(e -> System.err.println("--- ReviewService: deleteReview 오류 중 삭제 실패 - " + e.getMessage() + " ---"));
                })
                .onErrorResume(e -> {
                    // findById, 권한 확인, deleteById 등 체인 중 발생한 오류를 잡습니다.
                    System.err.println("--- ReviewService: deleteReview 오류 - " + e.getMessage() + " ---");
    ...
    //             .defaultIfEmpty(0.0); // 리뷰가 없을 경우 기본값 0.0 반환
    // }
}