...
import 'package:localy_front_flutter/presentation/widgets/cart_item_card.dart'; // CartItemCard 임포트
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (cartProvider.cart == null && !cartProvider.isLoading) {
        debugPrint("CartScreen initState: cart is null and not loading, calling fetchCart(force: true).");
        cartProvider.fetchCart(force: true);
      } else {
        debugPrint("CartScreen initState: cart is already loaded or loading. 아이템 수: ${cartProvider.cart?.items.length ?? 'N/A'}");
      }
    });
  }
    ...
    if (mounted) {
      if (orderResult != null) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text("주문이 성공적으로 완료되었습니다! (주문 ID: ${orderResult.orderId})"),
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("주문 실패: ${cartProvider.errorMessage}"), backgroundColor: Colors.redAccent, duration: const Duration(seconds: 3)),
        );
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("주문 처리 중 알 수 없는 오류가 발생했습니다."), backgroundColor: Colors.redAccent, duration: const Duration(seconds: 3)),
        );
      }
    }
  }
              ...
              onPressed: () async {
                ...
                final confirmClear = await showDialog<bool>(
                  ...
                  builder: (ctx) => AlertDialog(
                    ...
                    actions: <Widget>[
                      ...
                      TextButton(child: const Text("확인", style: TextStyle(color: Colors.red)), onPressed: () => Navigator.of(ctx).pop(true)),
                    ],
                  ),
                );
                if (confirmClear == true) {
                  ...
                  if (mounted && cartProvider.errorMessage == null) {
                    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("장바구니를 비웠습니다.")));
                  } else if (mounted && cartProvider.errorMessage != null) {
                    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("장바구니 비우기 실패: ${cartProvider.errorMessage}")));
                  }
                }
              },
            ),
            ...
            builder: (context) {
              ...
              return Column(
                children: [
                  ...
                  Expanded(
                    child: ListView.builder( // ListTile 대신 CartItemCard 사용
                      itemCount: items.length,
                      itemBuilder: (ctx, index) {
                        final item = items[index];
                        return CartItemCard(item: item); // CartItemCard 위젯 사용
                      },
                      // separatorBuilder는 Card의 margin으로 간격 조절되므로 제거하거나 유지 가능
                    ),
                  ),
                  // 주문하기 버튼 영역 (변경 없음)
                  Container(
